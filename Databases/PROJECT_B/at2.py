...
neighbourhoods.csv = sys.argv[1]
print('csv file: ' + csv_file)

NEIGHBORHOODS, extension = neighbourhoods.csv.rsplit('.', 1)

if NEIGHBOURHOODS.find('/') > -1:
    C:\Plato\B.B\airbnb dataset\neighbourhoods.csv, name = NEIGHBOURHOODS.rsplit('/', 1)
    slashes = '/'
elif NEIGHBOURHOODS.find('\\') > -1:
    C:\Plato\B.B\airbnb dataset\neighbourhoods.csv, name = NEIGHBOURHOODS.rsplit('\\', 1)
    slashes = '\\'
else:
    name = NEIGHBOURHOODS
...
NEIGHBOURHOODS = name.title()
print('table name: ' + NEIGHBOURHOODS)
